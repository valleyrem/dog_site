...
## Превью для мессенджеров (Open Graph)

При шаринге ссылки Telegram/WhatsApp/VK/FB читают OG-теги из `base.html`:

- страницы пород — фото породы (`photo_medium`, приоритетно)
- все остальные страницы — дефолтная обложка `static/og-cover.jpg`

Требования к обложке: **1200×630** (универсальный стандарт FB/Telegram/WhatsApp),
минимум 600×600 для крупной превьюшки Telegram. В шаблоне объявлены
`og:image:width/height` — без них часть краулеров не рисует большую карточку.

`ShowPost` в `views.py` не приклеивает хост к `photo_medium.url` из S3-хранилища
(оно уже возвращает абсолютный URL) — склейка ломала ссылку в `og:image`.

Кеш мессенджеров агрессивен: если ссылку шарили до правок, старое превью
прилипнет. Сброс:

- Telegram: добавить к URL `?v=2` или написать боту `@webpagebot`
